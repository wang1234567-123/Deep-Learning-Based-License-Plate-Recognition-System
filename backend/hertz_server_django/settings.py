...
INSTALLED_APPS = [
    ...
    'hertz_studio_django_lpr',  # 车牌识别模块
...
NO_AUTH_PATTERNS = config(
    'NO_AUTH_PATTERNS',
    default=r'^/api/auth/login/?$,^/api/auth/register/?$,^/api/auth/email/code/?$,^/api/auth/send-email-code/?$,^/api/auth/password/reset/?$,^/api/auth/token/refresh/?$,^/api/captcha/.*$,^/api/docs/.*$,^/api/redoc/.*$,^/api/schema/.*$,^/admin/.*$,^/static/.*$,^/media/.*$,^/demo/.*$,^/websocket/.*$,^/api/system/.*$,^/api/yolo/detect/?$',
    cast=lambda v: [s.strip() for s in v.split(',')]
)
