...
websocket_urlpatterns = list(demo_routing.websocket_urlpatterns)

if 'hertz_studio_django_lpr' in settings.INSTALLED_APPS:
    from hertz_studio_django_lpr import routing as lpr_routing

    websocket_urlpatterns += list(lpr_routing.websocket_urlpatterns)

if 'hertz_studio_django_yolo' in settings.INSTALLED_APPS:
    from hertz_studio_django_yolo import routing as yolo_routing

    websocket_urlpatterns += list(yolo_routing.websocket_urlpatterns)
