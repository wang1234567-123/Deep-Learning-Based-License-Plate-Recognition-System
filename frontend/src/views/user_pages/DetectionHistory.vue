  ...
  <div class="detection-history-page">
    ...
    <div class="history-content">
      <a-spin :spinning="loading">
        ...
        <!-- 布局1：网格布局（默认） -->
        <div v-else-if="currentLayout === 'default'" :key="`grid-${layoutKey}`" class="history-grid">
          <div 
            ...
            <div class="item-card">
              ...
              <div class="media-section">
                <div class="media-container">
                  <div v-if="record.detection_type === 'video'" class="media-thumbnail video-thumbnail">
                    <video 
                    ...
                    <div class="media-overlay">
                      ...
                      <div class="overlay-stats">
                        <div class="stat-item">
                          <span class="stat-number">{{ record.object_count || record.detections.length }}</span>
                ...
                <div class="item-header">
                  <div class="file-info">
                    ...
                    <div class="item-meta">
                      ...
                      <span class="meta-item" v-if="record.plate_number">
                        <AimOutlined />
                        {{ record.plate_number }}
                      </span>
                      <span class="meta-item">
                  ...
                  <div class="item-actions">
                    <a-button size="small" type="primary" @click.stop="viewDetails(record)" class="action-btn">
                      <EyeOutlined />
                      查看详情
                    </a-button>
                    <a-popconfirm
                ...
                <div class="detection-results">
                  ...
                  <div class="result-tags">
                    <a-tag 
                      ...
                      :key="index"
                      :color="getTagColor((detection as any).class || (detection as any).class_name)"
                      class="result-tag"
                    >
                      {{ (detection as any).class || (detection as any).class_name }} ({{ (detection.confidence * 100).toFixed(1) }}%)
                    </a-tag>
        ...
        <!-- 布局2：列表布局 -->
        <div v-else-if="currentLayout === 'list'" :key="`list-${layoutKey}`" class="history-list-layout">
          <div 
            v-for="record in detectionHistory" 
            :key="record.id"
            ...
            <div class="item-card-list">
              ...
              <div class="media-section-list">
                <div class="media-container-list">
                  <div v-if="record.detection_type === 'video'" class="media-thumbnail-list video-thumbnail">
                    <video 
                      :src="getFullFileUrl(record.original_file)" 
                      preload="metadata"
                      muted
            ...
            <div class="item-card-flow">
              ...
              <div class="media-section-flow">
                <div class="media-container-flow">
                  <div v-if="record.detection_type === 'video'" class="media-thumbnail-flow video-thumbnail">
                    <video 
                      :src="getFullFileUrl(record.original_file)" 
                      preload="metadata"
                      muted
        ...
        <div class="detail-content">
          <div v-if="selectedRecord" class="detail-layout">
            ...
            <div class="detail-left">
              <div class="detail-media" style="display: flex; flex-direction: row; gap: 12px; align-items: flex-start;">
                <div class="media-item" style="flex: 1; text-align: center; min-width: 0;">
                  <h5 style="margin-bottom: 8px; font-size: 14px;">原文件</h5>
                    <!-- 视频播放器 -->
                    <div v-if="selectedRecord.detection_type === 'video'" style="width: 100%; max-width: 100%; overflow: hidden; position: relative;">
                      <video 
                        :src="getFullFileUrl(selectedRecord.original_file)" 
                        controls
                        preload="metadata"
                        style="width: 100%; max-height: 150px; border-radius: 8px; object-fit: contain;"
                      ...
                      <div style="position: absolute; top: 8px; right: 8px;">
                        <a-button size="small" type="primary" @click="openVideoInNewTab(selectedRecord.original_file)" style="opacity: 0.8;">
                          <EyeOutlined />
                          全屏查看
                        </a-button>
          </div>
                      <div style="margin-top: 8px;">
                        <a-button size="small" @click="downloadVideo(selectedRecord.original_file, selectedRecord.original_filename)">
                          <DownloadOutlined />
                          下载原视频
                        </a-button>
        </div>
                    ...
                    <div v-else style="width: 100%; max-width: 100%; overflow: hidden;">
                      <div style="width: 100%; max-height: 150px; border: 2px dashed #d9d9d9; border-radius: 8px; display: flex; align-items: center; justify-content: center; background: #fafafa; overflow: hidden; position: relative;">
                        <a-image
                          :src="getFullFileUrl(selectedRecord.original_file)"
                          :alt="selectedRecord.original_filename"
                          style="max-width: 100%; max-height: 100%; object-fit: contain; width: 100%; height: auto; cursor: pointer; border-radius: 4px;"
                          :preview="{
                            mask: '点击放大',
                            zIndex: 10002,
                        ...
                        />
                        <div style="position: absolute; top: 4px; left: 4px; background: rgba(0,0,0,0.6); color: white; padding: 2px 6px; border-radius: 4px; font-size: 12px; pointer-events: none;">
                          <EyeOutlined /> 点击放大
                        </div>
                      </div>
                      <div style="margin-top: 8px;">
                        <a-button size="small" @click="downloadImage(selectedRecord.original_file, selectedRecord.original_filename)">
                          <DownloadOutlined />
                          下载原图
                        </a-button>
                        <a-button size="small" @click="openImageInNewTab(selectedRecord.original_file)" style="margin-left: 8px;">
                          <EyeOutlined />
                          新窗口查看
                        </a-button>
                      </div>
                ...
                <div class="media-item" style="flex: 1; text-align: center; min-width: 0;">
                  <h5 style="margin-bottom: 8px; font-size: 14px;">检测结果</h5>
                    ...
                    <div v-if="selectedRecord.detection_type === 'video'" style="width: 100%; max-width: 100%; overflow: hidden; position: relative;">
                      ...
                      <div style="position: absolute; top: 8px; right: 8px;">
                        <a-button size="small" type="primary" @click="openVideoInNewTab(selectedRecord.result_file)" style="opacity: 0.8;">
                          <EyeOutlined />
                          全屏查看
                        </a-button>
                      </div>
                      <div style="margin-top: 8px;">
                        <a-button size="small" @click="downloadVideo(selectedRecord.result_file, selectedRecord.result_filename)">
                          <DownloadOutlined />
                          下载结果视频
                        </a-button>
                      </div>
                    ...
                    <div v-else style="width: 100%; max-width: 100%; overflow: hidden;">
                      ...
                      <div style="margin-top: 8px;">
                        <a-button size="small" @click="downloadImage(selectedRecord.result_file, selectedRecord.result_filename)">
                          <DownloadOutlined />
                          下载结果图
                        </a-button>
                        <a-button size="small" @click="openImageInNewTab(selectedRecord.result_file)" style="margin-left: 8px;">
                          <EyeOutlined />
                          新窗口查看
                        </a-button>
                      </div>
              ...
              <h4>{{ selectedRecord.filename }}</h4>
                <div class="info-grid">
                  <div class="info-item">
                    ...
                    <span class="info-value">{{ formatDate(new Date(selectedRecord.created_at)) }}</span>
                  </div>
                  <div class="info-item">
                    <span class="info-label">检测目标数：</span>
                    <span class="info-value">{{ selectedRecord.object_count || selectedRecord.detections.length }} 个</span>
                  </div>
                  <div class="info-item">
                  ...
                  <div class="info-item">
                    ...
                    <span class="info-value">{{ selectedRecord.detection_type === 'image' ? '图片' : '视频' }}</span>
                  </div>
                  <div class="info-item" v-if="selectedRecord.plate_number">
                    <span class="info-label">识别车牌：</span>
                    <span class="info-value">{{ selectedRecord.plate_number }}</span>
                  </div>
                  <div class="info-item" v-if="selectedRecord.plate_confidence !== null && selectedRecord.plate_confidence !== undefined">
                    <span class="info-label">车牌置信度：</span>
                    <span class="info-value">{{ (selectedRecord.plate_confidence * 100).toFixed(1) }}%</span>
                  </div>
                  <div class="info-item" v-if="selectedRecord.avg_confidence">
                    <span class="info-label">平均置信度：</span>
                    <span class="info-value">{{ (selectedRecord.avg_confidence * 100).toFixed(1) }}%</span>
                  </div>
                </div>
            </div>
            ...
            <div class="detail-detections">
              ...
              <div class="detection-table">
                <template v-if="selectedRecord.detected_categories && selectedRecord.detected_categories.length > 0">
                  <a-table
...
const loadDetectionHistory = async () => {
  ...
  try {
    ...
    const response: any = await detectionHistoryApi.getUserDetectionHistory(
    ...
    if (response.success && response.data && (response.data as any).records) {
      console.log('📊 标准格式响应')
      rawData = (response.data as any).records
      totalRecords.value = (response.data as any).total || 0
  ...
  try {
    ...
    if (detailData) {
      ...
      const detailRecord = {
        ...
        // 构建检测结果数组
        detections: detailData.detected_categories?.map((category: string, index: number) => ({
          class_name: category,
...
const isAllSelected = computed(() => {
  return detectionHistory.value.length > 0 && selectedRecordIds.value.length === detectionHistory.value.length
})

const isIndeterminate = computed(() => {
  return selectedRecordIds.value.length > 0 && selectedRecordIds.value.length < detectionHistory.value.length
})
    ...
    onOk: async () => {
      ...
      try {
        ...
        const response = await detectionHistoryApi.batchDeleteDetectionRecords(
          userStore.userInfo.user_id,
          selectedRecordIds.value.map(String)
        )
...
const handleImagePreview = () => {
  // 延迟执行，确保预览组件已经渲染
  setTimeout(() => {
    const previewElements = document.querySelectorAll('.ant-image-preview, .ant-image-preview-wrap, .ant-image-preview-mask, .ant-image-preview-body, .ant-image-preview-img, .ant-image-preview-operations')
