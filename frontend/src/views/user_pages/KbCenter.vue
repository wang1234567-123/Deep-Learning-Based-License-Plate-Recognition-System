          ...
          <div class="qa-chat-card">
            ...
            <div class="qa-messages" ref="qaMessagesRef">
              <template v-for="msg in qaMessages" :key="msg.id">
                ...
                <!-- 加载状态气泡 -->
                <div v-else-if="msg.type === 'loading'" class="qa-message-row qa-message-row-answer">
        ...
        <div class="graph-container">
          ...
          <div class="graph-header-row">
            <div class="header-left">
              ...
              <span class="section-desc">实体和关系的可视化展示，支持自动抽取和手动管理 | 🖱️ 可拖拽节点</span>
            </div>
          ...
          <div class="graph-main-content">
            ...
            <div class="graph-canvas-card">
              ...
              <div class="canvas-footer">
                <span class="footer-tip">🖱️ 拖拽节点可移动 | 点击节点查看详情 | {{ entities.length }} 个实体, {{ relations.length }} 个关系</span>
              </div>
            </div>
              ...
              <div class="detail-content" v-if="selectedEntity">
                ...
                <div class="detail-group" v-if="selectedEntity.properties">
                  ...
                  <div class="props-list">
                    <div v-for="(val, key) in (typeof selectedEntity.properties === 'string' ? JSON.parse(selectedEntity.properties) : selectedEntity.properties)" :key="key" class="prop-item">
                ...
                <div class="detail-group">
                  ...
                  <div class="relations-list">
                    ...
                    <div v-else v-for="rel in selectedEntityRelations" :key="rel.id" class="rel-item">
                      <LinkOutlined />
                      <span class="rel-type">{{ rel.relation_type }}</span>
                      <ArrowRightOutlined class="arrow" />
                      <span class="rel-target">
                        {{ rel.target === selectedEntity.id ? getEntityName(rel.source) : getEntityName(rel.target) }}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div class="detail-actions">
                  ...
                  <a-popconfirm title="确定删除该实体？" @confirm="handleDeleteEntity(selectedEntity.id)">
          ...
          <div class="extract-card">
             ...
             <div class="extract-body">
                <a-input-group compact>
                   <a-input v-model:value="extractText" style="width: calc(100% - 200px)" placeholder="输入文本自动抽取..." />
                   <a-button type="primary" :loading="extractLoading" @click="handleExtractFromText">抽取</a-button>
                </a-input-group>
             </div>
          </div>
    ...
    >
      <a-form :model="itemForm" layout="vertical" class="kb-item-form">
        ...
        <a-form-item label="内容来源">
          <a-radio-group v-model:value="itemForm.source_type" button-style="solid" class="source-group">
    ...
    >
      <a-form :model="relationForm" layout="vertical">
        ...
        <a-form-item label="关系类型" required>
          <a-input v-model:value="relationForm.relation_type" placeholder="例如：related_to、depends_on 等" />
        </a-form-item>
...
const handleSaveEntity = async () => {
  ...
  try {
    ...
    message.error(
      e?.message ||
        (entityModalMode.value === 'create' ? '创建实体失败' : '更新实体失败')
    )
  } finally {
...
const updateForces = () => {
  ...
  nodes.forEach(node => {
    ...
    if (node.x > canvasWidth - PADDING) { node.x = canvasWidth - PADDING; node.vx = 0 }
    if (node.y < PADDING) { node.y = PADDING; node.vy = 0 }
    if (node.y > canvasHeight - PADDING) { node.y = canvasHeight - PADDING; node.vy = 0 }
  })
}
...
watch(activeTab, (key) => {
  if (key === 'items' && !items.value.length) fetchItems()
          ...
          .detail-group {
            ...
            .props-list {
              ...
              .prop-item {
                ...
                .prop-val { color: #334155; font-family: monospace; }
              }
            }
