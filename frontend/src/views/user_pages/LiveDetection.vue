<template>
  <div class="live-detection-page">
          ...
          <a-card title="摄像头控制" class="control-card">
            <a-space direction="vertical" style="width: 100%;">
              <div class="camera-preview">
                ...
                <div 
                  v-if="inferenceMode === 'onnx' && isCameraActive"
                  class="overlay-hud"
                >
            ...
            <a-space direction="vertical" style="width: 100%;">
                  ...
                  <a-form-item label="ONNX模型">
                    <div class="onnx-controls">
                      ...
                      <div class="row status">
                        ...
                        <span v-if="selectedOnnxModel" class="path-text">{{ selectedOnnxModel }}</span>
                      </div>
                    </div>
                  </a-form-item>
              ...
              <a-space>
                <a-button 
                  ...
                  :disabled="!isCameraActive || isDetecting || (inferenceMode === 'onnx' && !onnxModelLoaded)"
                ...
                <a-button @click="openLivePreview" :disabled="!isCameraActive">
                  放大预览
                </a-button>
                <a-button 
                  @click="clearResults"
              ...
              <div class="connection-status">
                <a-tag v-if="inferenceMode === 'pt'" :color="isConnected ? 'success' : 'error'" class="status-tag">
                  {{ isConnected ? '已连接' : '未连接' }}
                </a-tag>
                <a-tag v-if="inferenceMode === 'onnx'" :color="onnxModelLoaded ? 'success' : 'error'" class="status-tag">
                  {{ onnxModelLoaded ? '模型已加载' : '模型未加载' }}
                </a-tag>
            ...
            <div class="latest-result-content">
                ...
                <div class="result-image">
                  ...
                  <div class="image-overlay-info">
                    <div class="overlay-stats">
                      ...
                      <div class="stat-item" v-if="latestResult?.avg_confidence !== null && latestResult?.avg_confidence !== undefined">
                        <span class="stat-icon">📊</span>
                        <div class="stat-content">
                          <span class="stat-number">{{ (latestResult.avg_confidence * 100).toFixed(1) }}%</span>
                      ...
                      <div class="stat-item" v-if="latestResult?.plate_number">
                        <span class="stat-icon">🚗</span>
                        <div class="stat-content">
                          <span class="stat-number">{{ latestResult.plate_number }}</span>
                          <span class="stat-label">识别车牌</span>
                        </div>
                      </div>
                      <div class="stat-item" v-if="latestResult?.plate_confidence !== null && latestResult?.plate_confidence !== undefined">
                        <span class="stat-icon">🔤</span>
                        <div class="stat-content">
                          <span class="stat-number">{{ (latestResult.plate_confidence * 100).toFixed(1) }}%</span>
                          <span class="stat-label">车牌置信度</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div class="result-details">
                <div class="detection-summary">
                  ...
                  <div class="detection-tags" v-if="latestResult?.categories && latestResult.categories.length > 0">
                    <a-tag 
                    ...
                    >
                      ...
                      <span v-if="latestResult?.confidence_scores && latestResult.confidence_scores[idx]" class="tag-score">
    ...
    >
      <div v-if="selectedResult" class="detail-content">
        <div class="detail-image">
          <img 
            v-if="selectedResult.frame" 
            :src="selectedResult.frame" 
            alt="检测详情"
          />
          <div v-else class="image-placeholder">
            <FileImageOutlined />
        ...
        <a-descriptions :column="2" bordered>
          ...
          <a-descriptions-item label="平均置信度" :span="2">
            {{ selectedResult.avg_confidence !== null ? (selectedResult.avg_confidence * 100).toFixed(2) + '%' : 'N/A' }}
          </a-descriptions-item>
          <a-descriptions-item label="识别车牌" :span="2" v-if="selectedResult.plate_number">
            {{ selectedResult.plate_number }}
          </a-descriptions-item>
          <a-descriptions-item label="车牌置信度" :span="2" v-if="selectedResult.plate_confidence !== null && selectedResult.plate_confidence !== undefined">
            {{ (selectedResult.plate_confidence * 100).toFixed(1) }}%
          </a-descriptions-item>
          <a-descriptions-item label="检测类别" :span="2">
            <div class="detail-detections">
              ...
              >
                {{ category }}: 
                <span v-if="selectedResult.confidence_scores && selectedResult.confidence_scores[idx]">
    ...
    >
      <a-form layout="vertical">
        <a-form-item label="模型文件（.pt格式）" required>
          ...
          <div v-if="fileError" style="margin-top: 8px; color: #ff4d4f;">
            ❌ {{ fileError }}
          </div>
        </a-form-item>
        ...
        <a-form-item v-if="onnxConverting || onnxConversionStatus">
          <div style="padding: 16px; background: #f0f9ff; border-radius: 8px; border: 1px solid #91d5ff;">
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 8px;">
              <a-spin :spinning="onnxConverting" />
              <span style="font-weight: 600; color: #1890ff;">{{ onnxConversionStatus || '准备转换...' }}</span>
            </div>
            <a-progress 
              v-if="onnxConverting" 
              ...
              :show-info="true"
            />
            <div v-if="onnxConversionStatus && !onnxConverting" style="color: #52c41a; font-size: 14px;">
    ...
    <!-- 直播预览放大 -->
    <a-modal v-model:visible="livePreviewVisible" title="摄像头预览" width="80%" :footer="null" @cancel="closeLivePreview">
      <div style="display:flex;justify-content:center;align-items:center;background:#000;">
        <video ref="modalVideo" style="width:100%;max-height:70vh;object-fit:contain;" autoplay playsinline></video>
      </div>
    </a-modal>
  </div>
</template>
...
const closeLivePreview = () => {
  if (modalVideo.value) {
    try { modalVideo.value.pause?.() } catch {}
  }
}
...
const handleOnnxUpload = async () => {
  ...
  try {
    ...
    if (convertResponse.success && convertResponse.data) {
      ...
      // 获取转换后的文件信息
      const onnxUrl = convertResponse.data.download_url || 
                     convertResponse.data.onnx_url || 
                     convertResponse.data.onnx_path ||
                     convertResponse.data.onnx_relative_path
      const onnxFileName = convertResponse.data.file_name || 
                          `${onnxUploadForm.value.name}_${onnxUploadForm.value.version}.onnx`
      
      // 获取labels.json的URL
      const labelsUrl = convertResponse.data.labels_download_url || 
                       convertResponse.data.labels_relative_path
        ...
        try {
          ...
          if (labelsResponse.ok) {
            ...
            if (Array.isArray(labelsData)) {
              ...
              // 如果是对象，尝试提取值
              modelClasses = Object.values(labelsData).filter(v => typeof v === 'string') as string[]
            }
...
const loadOnnxModelList = async () => {
  try {
    ...
    if (manifestRes.ok) {
      ...
      files = Array.isArray(manifest) ? manifest : (Array.isArray(manifest?.files) ? manifest.files : [])
    }
...
const loadOnnxModel = async () => {
  ...
  try {
    ...
    // 先验证文件是否存在
      ...
      if (!testResponse.ok) {
        throw new Error(`模型文件不存在 (状态码: ${testResponse.status})。请确保文件在 public/models/ 目录下。`)
      }
      ...
      for (const url of candidates) {
        try {
          ...
          if (r.ok) {
            ...
            else if (typeof data === 'object') classNames = Object.values(data).filter(v => typeof v === 'string') as string[]
