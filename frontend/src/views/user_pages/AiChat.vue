    ...
    >
      <template #description>
        <a-descriptions :column="1" size="small">
          <a-descriptions-item label="状态码">{{ errorInfo.status ?? '未知' }}</a-descriptions-item>
          <a-descriptions-item label="请求方法">{{ errorInfo.method || 'GET' }}</a-descriptions-item>
                      ...
                      <a-list-item-meta>
                        <template #title>
                          <div class="chat-title-row">
                            ...
                            <a-space>
                              <a-button size="small" type="text" @click.stop="openRename(item)" class="edit-btn">
                                <EditOutlined />
                              </a-button>
                              <a-popconfirm title="确认删除该对话？" @confirm="deleteChat(item.id)">
                                <a-button size="small" danger type="text" @click.stop class="delete-btn">
                                  <DeleteOutlined />
                                </a-button>
                              </a-popconfirm>
                            </a-space>
                  ...
                  <template v-if="messages.length">
                    <div v-for="m in messages" :key="m.id" :class="['msg', m.role]">
                      <div class="message-avatar">
                        <a-avatar :size="32" :style="{ backgroundColor: m.role === 'user' ? '#3b82f6' : '#10b981' }">
                          <template #icon>
            ...
            <div class="chat-list-horizontal">
              ...
              >
                <template #renderItem="{ item }">
                  ...
                  >
                    <a-card :class="{ 'active-card': item.id === currentChatId }" size="small" hoverable>
                      <div class="horizontal-chat-title">{{ item.title }}</div>
                      <div class="horizontal-chat-desc">{{ item.latest_message || '暂无消息' }}</div>
                    </a-card>
                  </a-list-item>
                </template>
              </a-list>
        ...
        <div class="chat-container-wide">
          ...
          <div class="chat-sidebar-floating" :class="{ visible: sidebarVisible }">
            <div class="sidebar-header">
              ...
              <h3 class="sidebar-title">我的对话</h3>
              <a-button type="text" shape="circle" class="close-sidebar" @click="sidebarVisible = false">
                <template #icon>
                  <CloseOutlined />
                </template>
              </a-button>
...
const selectChat = async (id: number) => {
  if (typeof id !== 'number' || Number.isNaN(id) || id <= 0) {
    errorInfo.value = { status: 404, message: '无效的聊天ID', url: `/api/ai/chats/${String(id)}/`, method: 'GET' }
...
const createChat = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      await fetchChats()
      const newId = typeof (res.data as any)?.id === 'number' ? (res.data as any).id : Number((res.data as any)?.chat_id)
      if (typeof newId === 'number' && newId > 0) {
        selectChat(newId)
      } else {
        errorInfo.value = { status: null, message: '创建成功但未返回有效ID', url: '/api/ai/chats/create/', method: 'POST', data: res.data }
      }
    } else {
...
const onEnterSend = () => { if (canSend.value) send() }
    ...
    .chat-main {
      ...
      .chat-header {
        ...
        .header-icon {
          width: 36px;
          height: 36px;
          background: linear-gradient(135deg, var(--theme-primary, #10b981) 0%, var(--theme-primary, #059669) 100%);
          border-radius: 8px;
          display: flex;
          align-items: center;
          justify-content: center;
