  ...
  <div class="alert-processing-center">
    ...
    >
      <div class="batch-process-content">
        <p>已选择 <strong>{{ selectedRowKeys.length }}</strong> 条告警记录</p>
...
const pendingAlerts = computed(() => alerts.value.filter(alert => alert.status === 'pending').length)
const confirmedAlerts = computed(() => alerts.value.filter(alert => alert.status === 'is_confirm').length)
const falsePositiveAlerts = computed(() => alerts.value.filter(alert => alert.status === 'false_positive').length)
...
const confirmBatchProcess = async () => {
  try {
    ...
    if (successCount === selectedRowKeys.value.length) {
      ...
      message.warning(`处理完成，成功 ${successCount} 条，失败 ${selectedRowKeys.value.length - successCount} 条`)
    }
...
.alert-processing-center {
      ...
      .detection-info {
        ...
        .detection-image {
          :deep(.ant-image) {
            ...
            .ant-image-mask { border-radius: 8px; }
          }
        }
  ...
  .alert-detail {
    ...
    .image-comparison {
      ...
      .image-container {
        text-align: center;
        h5 { margin-bottom: 8px; color: #86868b; }
        .comparison-image {
          width: 100%;
          max-width: 300px;
          max-height: 200px;
  ...
  :deep(.ant-pagination) {
    ...
    .ant-select {
      margin: 0 8px;
      .ant-select-selector { border-radius: 8px; border: 0.5px solid rgba(0,0,0,0.12); background: rgba(255,255,255,0.8); &:hover { border-color: #3b82f6; } }
    }
  }
}
...
@media (max-width: 768px) {
  .alert-processing-center {
    ...
    .alerts-card { margin: 0 16px 20px 16px; border-radius: 12px; }
    .alerts-table .action-buttons { flex-direction: column; gap: 6px; }
  }
}
</style>
