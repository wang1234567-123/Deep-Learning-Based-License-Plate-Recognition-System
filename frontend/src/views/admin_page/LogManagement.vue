...
async function fetchList() {
  ...
  try {
    ...
    // 统计状态分布（基于所有数据）
    statusStats.success = allRows.filter(r => r.response_status >= 200 && r.response_status < 300).length
    statusStats.clientError = allRows.filter(r => r.response_status >= 400 && r.response_status < 500).length
