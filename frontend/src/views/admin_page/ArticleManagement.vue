  ...
  <div class="knowledge-base">
    ...
    <a-drawer v-model:visible="detailOpen" title="文档详情" width="720" :destroy-on-close="true">
      <a-descriptions bordered :column="1" v-if="currentDoc">
        ...
        <a-descriptions-item label="发布时间">{{ currentDoc.published_at || '-' }}</a-descriptions-item>
        <a-descriptions-item label="标签" v-if="currentDoc.tags_list && currentDoc.tags_list.length">
          <a-space wrap>
            <a-tag v-for="tag in currentDoc.tags_list" :key="tag">{{ tag }}</a-tag>
          </a-space>
        </a-descriptions-item>
        <a-descriptions-item label="摘要" v-if="currentDoc.summary">{{ currentDoc.summary }}</a-descriptions-item>
    ...
    >
      <a-form :model="editForm" layout="vertical">
        ...
        <a-form-item label="分类" required>
          <a-select v-model:value="editForm.category" :options="categoryOptions" placeholder="请选择分类" />
        </a-form-item>
        <a-form-item label="标签">
          <a-select v-model:value="editForm.tagsArray" mode="tags" placeholder="输入并回车添加标签" />
        </a-form-item>
...
const handleSave = async () => {
  ...
  try {
    ...
    if (d?.detail && Array.isArray(d.detail)) {
      const msgs = d.detail.map((it: any) => `${(it.loc?.[it.loc.length - 1]) || '字段'}: ${it.msg || it.message || '验证失败'}`)
