                ...
                <div class="task-card-header">
                  <div class="title-block">
                    ...
                    <div class="subtitle-row">
                      ...
                      <span class="dot">•</span>
                      <span class="model-info">YOLO{{ (job.model_family || '').toUpperCase() }} {{ job.model_size ? job.model_size.toUpperCase() : '' }}</span>
                      <span class="dot">•</span>
                      <span class="created-time">{{ formatDateTime(job.created_at) }}</span>
                    </div>
                  </div>
                  <div class="progress-value">
                    {{ displayProgress(job).toFixed(0) }}%
                  </div>
                </div>
        ...
        <div class="task-detail-view">
          <div class="detail-header">
            ...
            <div class="detail-title-block">
              ...
              <div class="sub-row">
                ...
                <span class="dot">•</span>
                <span>YOLO{{ (currentJob.model_family || '').toUpperCase() }} {{ currentJob.model_size ? currentJob.model_size.toUpperCase() : '' }}</span>
                <span class="dot">•</span>
            ...
            <div class="detail-actions">
              ...
              <a-button v-if="currentJob.status === 'completed'" type="primary" @click="handleDownload(currentJob.id)">
                下载结果
              </a-button>
              <a-popconfirm
                title="确认删除该训练任务及其输出文件？"
          ...
          <div class="status-cards">
            ...
            <div class="status-card">
              ...
              <div class="value">{{ renderDuration(currentJob.started_at, currentJob.finished_at) }}</div>
            </div>
          </div>
          ...
          <div class="time-section">
            ...
            <div class="time-grid">
              ...
              <div class="time-item">
                ...
                <div class="value">{{ currentJob.started_at ? formatDateTime(currentJob.started_at) : '未开始' }}</div>
              </div>
              <div class="time-item">
                <div class="label">结束时间</div>
                <div class="value">{{ currentJob.finished_at ? formatDateTime(currentJob.finished_at) : '未结束' }}</div>
              </div>
            </div>
          </div>
        </div>
                ...
                <a-col :xs="24" :md="12">
                  <a-form-item label="模型版本" required>
                    <a-select
                      ...
                      :options="versionOptions.map(v => ({ label: 'YOLO' + v.family.toUpperCase(), value: v.family }))"
                    />
                  </a-form-item>
                </a-col>
              </a-row>
...
const filteredJobs = computed(() => {
  ...
  if (statusFilter.value === 'running') {
    return jobs.value.filter(j => j.status === 'queued' || j.status === 'running' || j.status === 'canceling')
  }
