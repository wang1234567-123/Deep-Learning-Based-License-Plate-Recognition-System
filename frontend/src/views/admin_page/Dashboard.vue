<template>
  <div class="dashboard-container">
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card stat-card-notifications" :class="{ loading: systemLoading }">
        <div class="stat-card-header">
          ...
          <div class="stat-info">
            ...
            <div class="stat-value">{{ systemLoading ? '...' : `${systemStatus.cpuUsage}%` }}</div>
          </div>
        </div>
        <div class="stat-card-footer">
          <span class="stat-hint">当前CPU占用率</span>
        </div>
      </div>
      ...
      <div class="stat-card stat-card-logs" :class="{ loading: statsLoading }">
        <div class="stat-card-header">
          <div class="stat-icon-wrapper logs-icon">
            <ScanOutlined class="stat-icon" />
          </div>
          <div class="stat-info">
            <div class="stat-title">检测数</div>
            <div class="stat-value">{{ statsLoading ? '...' : dashboardStats.totalLogs }}</div>
          </div>
        </div>
        <div class="stat-card-footer">
          <span class="stat-hint">系统检测总次数</span>
        </div>
      </div>
      
      <div class="stat-card stat-card-knowledge" :class="{ loading: systemLoading }">
        <div class="stat-card-header">
          ...
          <div class="stat-info">
            ...
            <div class="stat-value">{{ systemLoading ? '...' : `${systemStatus.memoryUsage}%` }}</div>
          </div>
        </div>
        <div class="stat-card-footer">
          <span class="stat-hint">当前内存占用率</span>
        </div>
      </div>
    </div>
...
const quickActions = [
  ...
  //{ key: 'department', label: '部门管理', icon: PlusOutlined, gradient: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)' },
