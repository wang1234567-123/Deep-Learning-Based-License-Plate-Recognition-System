...
export const dashboardApi = {
  ...
  getRealStats: async (): Promise<ApiResponse<DashboardStats>> => {
    try {
      // 并行获取各种统计数据
      const [notificationStats, yoloStats, knowledgeStats] = await Promise.all([
        noticeUserApi.statistics().catch(() => ({ success: false, data: { total_count: 0, unread_count: 0 } })),
        request.get('/api/yolo/detections/stats/', { showError: false }).catch(() => ({ success: false, data: null })),
      ...
      const totalLogs = yoloStats?.success ? Number((yoloStats as any)?.data?.total_detections || 0) : 0
